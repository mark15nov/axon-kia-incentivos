...
import { bonoMatriz, bonoOfertas, dealers, dealerNombre, fmtMXN, ESTATUS } from '../../data/bonoVentas.js'

export default function BonoStep2Matriz() {
  const [filtro, setFiltro] = useState('todos')
  const [sel, setSel] = useState(null)

  const aclaraciones = useMemo(() => bonoMatriz.filter(r => r.estatus !== 'ok'), [])
  const filas = useMemo(() => filtro === 'todos' ? bonoMatriz : bonoMatriz.filter(r => r.estatus === filtro), [filtro])

  const grupos = dealers
    .map(d => ({ dealer: d, regs: filas.filter(r => r.dealer === d.id) }))
    .filter(g => g.regs.length > 0)

  const totalOK = bonoMatriz.filter(r => r.estatus === 'ok').length
  const montoOK = bonoMatriz.filter(r => r.estatus === 'ok').reduce((a, b) => a + b.bono, 0)

  return (
    <div className="space-y-6">
      <SectionTitle
        kicker="Paso 2 · Revisión financiera"
        title="Matriz VIN × Monthly Commercial Offer"
        desc="Cada VIN (vertical, agrupado por agencia) se cruza contra la oferta comercial del modelo (horizontal). Finanzas detecta errores de información y duplicaciones, y genera aclaraciones a completar antes de consolidar."
        right={
          <div className="flex gap-2">
            {[{ k: 'todos', t: 'Todos' }, { k: 'aclaracion', t: 'Aclaraciones' }, { k: 'duplicado', t: 'Duplicados' }, { k: 'no_apto', t: 'No aptos' }].map(b => (
      ...
      <div className="grid grid-cols-4 gap-4">
        <Kpi label="VINs en matriz" value={bonoMatriz.length} sub="Cruzados con oferta" />
        <Kpi label="Validados" value={totalOK} sub="Listos para consolidar" />
        <Kpi label="Aclaraciones" value={aclaraciones.length} sub="Requieren completar" accent />
        <Kpi label="Bono validado" value={fmtMXN(montoOK)} sub="Elegible a pago" />
      </div>
      ...
      <div className="grid grid-cols-3 gap-6 items-start">
        <Card className="col-span-2 overflow-hidden">
                ...
                <tr className="bg-slate-50 border-b border-kia-line">
                  ...
                  {bonoOfertas.map(o => (
                    <th key={o.id} className="px-2 py-3 text-center font-semibold text-kia-gray whitespace-nowrap">
                      <div className="text-[11px]">{o.modelo}</div>
                      <div className="text-[10px] text-slate-400 font-normal">{fmtMXN(o.bono)}</div>
                    </th>
                  ))}
                </tr>
              </thead>
          ...
          <div className="flex items-center gap-4 px-4 py-3 border-t border-kia-line text-xs text-kia-gray">
            ...
            <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-slate-400" /> No apto (curtailment)</span>
          </div>
        </Card>
          ...
          {sel ? (
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-xl bg-slate-50 border border-kia-line px-3.5 py-3 text-xs text-kia-gray mt-1">
                Toca una aclaración para ver el detalle. Estos casos se envían a la agencia para corregir antes de consolidar el archivo (Paso 3).
              </div>
            </div>
          )}
        </Card>
...
function FilaGrupo({ grupo, onSel, sel }) {
  return (
    <>
      <tr>
        <td colSpan={bonoOfertas.length + 1} className="px-4 pt-4 pb-1.5">
      ...
      {grupo.regs.map(r => (
        <tr key={r.id} className={`border-b border-slate-50 hover:bg-slate-50/60 ${sel?.id === r.id ? 'bg-amber-50/50' : ''}`}>
          ...
          {bonoOfertas.map(o => {
            if (r.oferta !== o.id) return <td key={o.id} className="text-center text-slate-200">·</td>
            const e = ESTATUS[r.estatus]
            return (
              <td key={o.id} className="text-center py-2.5">
                <button onClick={() => r.estatus !== 'ok' && onSel(r)}
                  ...
                  {fmtMXN(r.bono).replace('$', '').replace(/,000$/, 'k')}
                </button>
              </td>
            )
          })}
...
const ACCION = {
  duplicado: 'Eliminar el registro duplicado y conservar el de VQM original.',
  no_apto: 'Excluir del bono. El VIN tiene curtailment acumulado; no procede pago.',
  aclaracion: 'Solicitar a la agencia la corrección del dato y reenviar evidencia.'
}
const ACCION_BTN = {
  duplicado: 'Eliminar duplicado',
  no_apto: 'Confirmar exclusión',
  aclaracion: 'Enviar aclaración a la agencia'
}
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className={`rounded-xl ${e.bg} px-4 py-3`}>
        ...
        <div className="text-xs text-slate-600 mt-1">{dealerNombre(reg.dealer)} · {reg.modelo} · {reg.mesesInventario} meses inv.</div>
      </div>
      <div className="mt-3 space-y-2.5 text-sm">
        <Campo label="Hallazgo" valor={reg.nota} />
        <Campo label="Bono calculado" valor={fmtMXN(reg.bono)} />
        <Campo label="Acción sugerida" valor={ACCION[reg.estatus] || ACCION.aclaracion} />
      </div>
