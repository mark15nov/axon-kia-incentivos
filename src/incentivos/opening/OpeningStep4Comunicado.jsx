...
import { ofComunicadoBase, ofDealersAplican, OF_PERIODO } from '../../data/openingFee.js'
import { vmForecast } from '../../data/variableMargin.js'

// Toda la red nacional (derivada del universo de dealers del forecast).
const RED = vmForecast.map((d, i) => ({ id: 'R' + String(i + 1).padStart(2, '0'), dealer: d.dealer, zona: d.zona }))
const APLICAN = ofDealersAplican

const AUDIENCIAS = [
  { v: 'red', label: 'Toda la red', desc: 'Todos los concesionarios' },
  { v: 'aplican', label: 'Los que aplican', desc: 'Dealers del programa' },
  { v: 'sel', label: 'Seleccionados', desc: 'Elige manualmente' }
]

export default function OpeningStep4Comunicado() {
  const [audiencia, setAudiencia] = useState('aplican')
  const [seleccion, setSeleccion] = useState(() => APLICAN.map(d => d.id))
  const [asunto, setAsunto] = useState(ofComunicadoBase.asunto)
  const [cuerpo, setCuerpo] = useState(ofComunicadoBase.cuerpo)
  const [enviados, setEnviados] = useState([])   // ids ya notificados
  ...
  const resetBase = () => { setAsunto(ofComunicadoBase.asunto); setCuerpo(ofComunicadoBase.cuerpo) }
  const editado = asunto !== ofComunicadoBase.asunto || cuerpo !== ofComunicadoBase.cuerpo

  return (
    <div className="space-y-6">
      <SectionTitle
        kicker="Paso 4 · Comunicado"
        title="Comunicado a dealers"
        desc="Redacta el comunicado sobre la base propuesta y envíalo a la audiencia que definas: a toda la red, solo a los dealers a los que aplica el Opening Fee, o a una selección manual."
          ...
          <Card className="p-5">
            ...
            <div className="rounded-xl border border-kia-line overflow-hidden">
              <div className="bg-kia-black text-white px-4 py-3">
                <div className="text-[11px] uppercase tracking-wider text-kia-red-soft font-semibold">KIA · Opening Fee</div>
                <div className="font-bold mt-0.5 text-sm">{asunto || <span className="text-white/40">Sin asunto</span>}</div>
              </div>
              <div className="px-4 py-4 text-sm text-slate-700 leading-relaxed whitespace-pre-line">
                {cuerpo}
              </div>
            </div>
          </Card>
      ...
      <div className={`flex items-start gap-3 rounded-xl px-4 py-3.5 text-sm border ${todoEnviado ? 'bg-emerald-50 border-emerald-100 text-emerald-900' : 'bg-slate-100 border-kia-line text-slate-700'}`}>
        ...
        <p>{todoEnviado
          ? <>Comunicado enviado a <strong>{destinatarios.length}</strong> dealer(s) de la audiencia «{AUDIENCIAS.find(a => a.v === audiencia).label}» para {OF_PERIODO}.</>
          : <>Ajusta la redacción y la audiencia; el comunicado se enviará a <strong>{destinatarios.length}</strong> dealer(s) y se registrará para el periodo {OF_PERIODO}.</>}</p>
      </div>
    </div>
  )
}
