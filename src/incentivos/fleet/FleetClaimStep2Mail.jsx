...
import { fcComunicadoBase, fcDealersAplican, FC_PERIODO } from '../../data/fleetClaim.js'
...
const APLICAN = fcDealersAplican

const AUDIENCIAS = [
  { v: 'red', label: 'Toda la red', desc: 'Todos los concesionarios' },
  { v: 'aplican', label: 'Los que aplican', desc: 'Dealers de flotilla' },
  { v: 'sel', label: 'Seleccionados', desc: 'Elige manualmente' }
]

export default function FleetClaimStep2Mail() {
  const [audiencia, setAudiencia] = useState('aplican')
  const [seleccion, setSeleccion] = useState(() => APLICAN.map(d => d.id))
  const [asunto, setAsunto] = useState(fcComunicadoBase.asunto)
  const [cuerpo, setCuerpo] = useState(fcComunicadoBase.cuerpo)
  ...
  const resetBase = () => { setAsunto(fcComunicadoBase.asunto); setCuerpo(fcComunicadoBase.cuerpo) }
  const editado = asunto !== fcComunicadoBase.asunto || cuerpo !== fcComunicadoBase.cuerpo

  return (
    <div className="space-y-6">
      <SectionTitle
        kicker="Paso 2 · Comunicado"
        title="Mail a dealers"
        desc="Redacta el comunicado del plan de flotilla sobre la base propuesta y envíalo a la audiencia que definas: a toda la red, solo a los dealers de flotilla o a una selección manual. En el correo se solicita a los dealers enviar su plan con las condiciones propuestas."
          ...
          <Card className="p-5">
            ...
            <div className="rounded-xl border border-kia-line overflow-hidden">
              <div className="bg-kia-black text-white px-4 py-3">
                <div className="text-[11px] uppercase tracking-wider text-kia-red-soft font-semibold">KIA · Fleet Claim</div>
      ...
      <div className={`flex items-start gap-3 rounded-xl px-4 py-3.5 text-sm border ${todoEnviado ? 'bg-emerald-50 border-emerald-100 text-emerald-900' : 'bg-slate-100 border-kia-line text-slate-700'}`}>
        ...
        <p>{todoEnviado
          ? <>Comunicado enviado a <strong>{destinatarios.length}</strong> dealer(s). A partir del envío, los dealers remiten su plan de flotilla para revisión (Paso 3).</>
          : <>Ajusta la redacción y la audiencia; el comunicado se enviará a <strong>{destinatarios.length}</strong> dealer(s) para {FC_PERIODO}.</>}</p>
      </div>
    </div>
  )
}
