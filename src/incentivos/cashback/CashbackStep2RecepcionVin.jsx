...
// Clasificación contra el Inbursa Report of Downpayment (Reporte de Enganche).
// Regla: si el VIN NO se encuentra en el reporte de enganche de Inbursa,
// se clasifica como Cashback. Los VIN sí encontrados llevan enganche Inbursa
// y se excluyen del cálculo de Cashback.
const CLASIF = { cashback: 117, enganche: 31 }

const PREVIEW = [
  { vin: '3KPA24AD1PE482103', folio: 'A-4821', dealer: 'KIA Polanco', modelo: 'Sportage', monto: 512000, clase: 'cashback' },
  { vin: 'KNADE163XP6218847', folio: 'B-1902', dealer: 'KIA Monterrey Valle', modelo: 'Seltos', monto: 438000, clase: 'enganche' },
  { vin: '3KPF54AD7PE097512', folio: 'A-4822', dealer: 'KIA Patria', modelo: 'K4', monto: 389000, clase: 'cashback' },
  { vin: 'KNAB2511BP7743120', folio: 'C-3310', dealer: 'KIA Querétaro', modelo: 'Rio', monto: 298000, clase: 'cashback' },
  { vin: '3KPA24AD9PE482210', folio: 'A-4823', dealer: 'KIA Cancún', modelo: 'Sportage', monto: 524000, clase: 'cashback' },
  { vin: 'KNADE163XP6219004', folio: 'B-1903', dealer: 'KIA Bajío', modelo: 'Seltos', monto: 442000, clase: 'enganche' }
]

const CLASE = {
  cashback: { tone: 'green', label: 'Cashback' },
  enganche: { tone: 'gray', label: 'Enganche Inbursa' }
}

export default function CashbackStep2RecepcionVin() {
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        ...
        desc="Sube el .zip con las facturas de los dealers (PDF + XML). El sistema escanea el paquete, parea PDF ↔ XML por VIN y clasifica cada VIN contra el Inbursa Report of Downpayment: si el VIN no se encuentra en el reporte de enganche, se considera Cashback."
        right={<Pill tone={estado === 'listo' ? 'green' : 'gray'}>
          {estado === 'listo' ? <><Icon.Check width={14} height={14} /> Escaneo completo</> : 'Pendiente de carga'}
        </Pill>}
      />

      {/* ---------- Dependencia · flujo de Downpayment (D01 / S04) ---------- */}
      <div className="flex items-start gap-3 rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3.5 text-sm text-emerald-900">
        ...
        <p>
          <strong>Flujo de Downpayment ejecutado.</strong> Los VIN ya fueron clasificados por el proceso de Downpayment, por lo que esta clasificación de Cashback puede continuar. Si el flujo de Downpayment no se hubiera ejecutado, el proceso mostraría una alerta y esperaría hasta completarlo.
        </p>
      </div>
      ...
      {estado === 'idle' && (
        ...
        >
          ...
          <div className="flex items-center justify-center gap-4 mt-5 text-xs text-kia-gray">
            ...
            <span className="flex items-center gap-1.5"><Icon.Database width={13} height={13} /> Clasificación vs Inbursa</span>
          </div>
        </div>
      )}
      ...
      {estado === 'escaneando' && (
        <Card className="p-6">
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-sm text-kia-gray">Leyendo paquete, pareando PDF ↔ XML y clasificando vs Inbursa…</span>
      ...
      {estado === 'listo' && (
        <div className="space-y-6 animate-fade-up">
          ...
          {/* ---------- Clasificación vs Inbursa Report of Downpayment ---------- */}
          <Card className="p-5">
            <div className="flex items-start gap-3 mb-4">
              <span className="h-10 w-10 shrink-0 rounded-xl bg-slate-100 grid place-items-center"><Icon.Database width={19} height={19} className="text-kia-black" /></span>
              <div className="min-w-0">
                <h3 className="font-bold">Clasificación vs Inbursa Report of Downpayment</h3>
                <p className="text-sm text-kia-gray mt-0.5">
                  Cada VIN se cruza contra el reporte de enganche de Inbursa. <strong className="text-kia-black">Si el VIN no se encuentra en el reporte, se clasifica como Cashback</strong>; los VIN presentes llevan enganche Inbursa y se excluyen.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 px-4 py-3.5">
                <div className="flex items-center gap-2">
                  <Icon.Check width={15} height={15} className="text-emerald-600" />
                  <span className="text-sm font-semibold text-emerald-800">Clasificados como Cashback</span>
                </div>
                <div className="text-2xl font-bold tabular text-emerald-700 mt-1">{CLASIF.cashback}</div>
                <div className="text-xs text-emerald-900/70">No encontrados en el reporte Inbursa</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3.5">
                <div className="flex items-center gap-2">
                  <Icon.Database width={15} height={15} className="text-kia-gray" />
                  <span className="text-sm font-semibold text-kia-black">Enganche Inbursa</span>
                </div>
                <div className="text-2xl font-bold tabular text-kia-black mt-1">{CLASIF.enganche}</div>
                <div className="text-xs text-kia-gray">Encontrados en el reporte · se excluyen</div>
              </div>
            </div>
          </Card>
              ...
              <table className="w-full text-sm whitespace-nowrap">
                <thead>
                  <tr className="bg-slate-50 text-kia-gray text-xs uppercase tracking-wide">
                    ...
                    <th className="text-left font-semibold px-3 py-3">Clasificación</th>
                  ...
                  {PREVIEW.map(f => (
                    <tr key={f.vin} className="border-t border-slate-100 hover:bg-slate-50/60">
                      ...
                      <td className="px-3 py-3">
                        <Pill tone={CLASE[f.clase].tone}>{CLASE[f.clase].label}</Pill>
                      </td>
          ...
          <div className="flex items-start gap-3 rounded-xl bg-sky-50 border border-sky-100 px-4 py-3.5 text-sm text-sky-900">
            ...
            <p>Se clasificaron <strong>{CLASIF.cashback} VIN como Cashback</strong> (no encontrados en el Inbursa Report of Downpayment). En el siguiente paso se cruzan contra el forecast y el soporte de cada dealer para confirmar el pago del incentivo.</p>
          </div>
        </div>
      )}
    </div>
