...
import {
  vmVins, vmVinResumen, vmVinMotivos, vmVinRechazosPorDealer,
  vmDealerCompliance, VM_BRAIN_RECHAZOS_METODO, VM_PERIODO, fmtMXN
...
const BRAIN_VALIDACIONES = [
  ...
  {
    titulo: 'Asignación por método de pago',
    desc: 'Clasifica cada VIN según su forma de pago para determinar la bolsa que aplica.'
  },
  {
    titulo: 'Cálculo del incentivo con oferta comercial',
    desc: 'Aplica la oferta comercial vigente para calcular el monto del bono por unidad.'
  },
  {
    titulo: 'Validación de no duplicados, aplicabilidad y uso del vehículo',
    desc: 'Descarta VIN duplicados y valida la aplicabilidad del incentivo y el uso del vehículo.'
  }
]
...
Tras la validación de VIN del periodo ${VM_PERIODO}, se rechazaron ${vins.length} unidad(es) del incentivo Cashback por los siguientes motivos:
...
export default function CashbackStep3ValidacionVin() {
  ...
  const porDealer = vmVinRechazosPorDealer()
  const rechazoDe = Object.fromEntries(porDealer.map(g => [g.dealer, g]))
  const compliance = vmDealerCompliance()
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {BRAIN_VALIDACIONES.map((v, i) => (
            <div key={i} className="flex items-start gap-3 rounded-xl border border-kia-line bg-slate-50/60 px-4 py-3.5">
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-kia-black">{v.titulo}</div>
                <div className="text-xs text-kia-gray mt-0.5 leading-snug">{v.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Kpi label="Monto a pagar · incentivo" value={fmtMXN(r.pago)} sub="Bono por unidad válida" accent />
        ...
        <Card className="lg:col-span-2 p-5">
          ...
          <div className="mt-4 pt-4 border-t border-kia-line grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-3 py-2.5">
              ...
              <div className="text-[11px] text-emerald-700/80">a pagar del incentivo</div>
            </div>
        ...
        {brainListo && peor && (
          <div className="p-5 bg-white animate-fade-up space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
              {VM_BRAIN_RECHAZOS_METODO.map((m, i) => (
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-kia-line space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <Pill tone="gray">{filtrados.length} de {vmVins.length}</Pill>
          </div>
